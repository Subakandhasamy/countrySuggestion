...
public interface CountyRepository extends JpaRepository<CountySuggestion, Long> {
  ...
  /**
   * Search for counties by exact name and state , ordered by FIPS code in ascending order.
   *
   ...
   */
  @Query("SELECT c FROM CountySuggestion c WHERE lower(c.name) = lower(:name) AND lower(c.state) = lower(:state) ORDER BY c.fips ASC")
  List<CountyProjection> findByNameIgnoreCaseAndStateIgnoreCaseOrderByFipsAsc(String name, String state);
  ...
  @Query("SELECT c FROM CountySuggestion c " +
          ...
          " OR lower(c.name) = lower(:name) OR lower(c.state) = lower(:state) " +
